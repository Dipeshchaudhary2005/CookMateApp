...
class FetchFromDatabase {
  static Future<List<String>> getDataList(String collection) async {
    final snap = await FirebaseFirestore.instance.collection(StaticClass.predefinedCollection)
        .doc(collection)
        .collection(PredefinedCollection.listOfFields)
        .get();


    return snap.docs.map((doc) => doc.get(PredefinedCollection.nameField) as String?)
